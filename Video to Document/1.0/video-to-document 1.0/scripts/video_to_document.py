...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", default="base", help="Whisper model name. Defaults to base.")
...
def transcribe_with_openai_whisper(audio_path: Path, model_name: str, language: str | None, include_timestamps: bool) -> str | None:
    try:
...
def transcribe_with_faster_whisper(audio_path: Path, model_name: str, language: str | None, include_timestamps: bool) -> str | None:
    try:
...
def transcribe_audio(audio_path: Path, model_name: str, language: str | None, include_timestamps: bool) -> str:
    transcript = transcribe_with_openai_whisper(audio_path, model_name, language, include_timestamps)
    if transcript:
        return transcript
    transcript = transcribe_with_faster_whisper(audio_path, model_name, language, include_timestamps)
    if transcript:
        return transcript
    raise SystemExit(
        "No supported local transcription package found. Install openai-whisper or faster-whisper, "
        "or rerun with --transcript-file."
    )
...
def main() -> int:
    ...
    try:
        if args.transcript_file:
            ...
            transcript = transcribe_audio(audio_path, args.model, args.language, not args.no_timestamps)
